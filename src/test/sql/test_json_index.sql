...
-- slow query
select * from test_table where json_get_timestamp(val, 'create_date') between '2009-12-04 01:00:00' and '2009-12-04 02:00:00';
...
--   Recheck Cond: ((json_get_timestamp(val, 'create_date'::text) >= '2009-12-04 01:00:00'::timestamp without time zone) AND (json_get_timestamp(val, 'create_date'::text) <= '2009-12-04 02:00:00'::timestamp without time zone))
--   ->  Bitmap Index Scan on test_date_idx  (cost=0.00..167.56 rows=7864 width=0) (actual time=0.015..0.015 rows=3 loops=1)
--         Index Cond: ((json_get_timestamp(val, 'create_date'::text) >= '2009-12-04 01:00:00'::timestamp without time zone) AND (json_get_timestamp(val, 'create_date'::text) <= '2009-12-04 02:00:00'::timestamp without time zone))
